...
// Label-buffer size, = 2*MAX_VARIABLE_LENGTH+40. Spelled as a literal because the
// seed tcc_cc's constant-expression evaluator has no '*' operator (expr_eval), so a
// multiplicative array dimension fails to compile when self-hosting on arm64.
#define MAX_LABEL_LENGTH 140
...
// Emit an unconditional branch to a label (loads &label into w16, br x16).
void emit_jump(FILE *fout, const char *label)
{
	fprintf(fout, "\tLOAD_W16_AHEAD\n\tSKIP_32_DATA\n\t&%s\n\tBR_X16\n", label);
}

// Emit a conditional branch to a label. cond is "EQ" or "NE": branch is taken
// when that condition holds. Implemented as an inverted fixed-offset skip over
// the load+br block (SKIP_INST_<inverse> jumps past the 4 following words).
void emit_branch(FILE *fout, const char *cond, const char *label)
{
	const char *skip = strcmp(cond, "EQ") == 0 ? "SKIP_INST_NE" : "SKIP_INST_EQ";
	fprintf(fout, "\t%s\n\tLOAD_W16_AHEAD\n\tSKIP_32_DATA\n\t&%s\n\tBR_X16\n", skip, label);
}
	...
	while (TRUE)
	{
		...
		else if (sym == 'B' || sym == 'D')
		{
			...
			}
			{
				...
				if (sym == 'B')
					sprintf(label, "_%s_loop_end%d", function_name, nesting_id[i]);
				else
					sprintf(label, "_%s_loop%d", function_name, nesting_id[i]);
				emit_jump(fout, label);
			}
		}
		else if (sym == 'I')
		{
			...
			{
				char label[MAX_LABEL_LENGTH];
				sprintf(label, "_%s_else%d", function_name, id);
				fprintf(fout, "\tCMP_X0_0              # if\n\tPOP_X0\n");
				emit_branch(fout, "EQ", label);
			}
			if (nesting_depth >= MAX_NESTING)
			{
				fprintf(ferr, "ERROR %d.%d: Nesting deeper than %d\n", cur_line, cur_column, MAX_NESTING);
		...
		else if (sym == '}')
		{
			...
			{
				char label[MAX_LABEL_LENGTH];
				sprintf(label, "_%s_loop%d", function_name, nesting_id[nesting_depth]);
				emit_jump(fout, label);
				fprintf(fout, ":_%s_loop_end%d\n", function_name, nesting_id[nesting_depth]);
			}
			else if (nesting_type[nesting_depth] == 'I')
			{
				...
				if (sym == 'E')
				{
					...
					{
						char label[MAX_LABEL_LENGTH];
						sprintf(label, "_%s_else_end%d", function_name, nesting_id[nesting_depth]);
						emit_jump(fout, label);
					}
		...
		else if (sym == 'G')
		{
			...
			{
				char label[MAX_VARIABLE_LENGTH+MAX_TOKEN_LENGTH+40];
				sprintf(label, "l_%s_%s", function_name, token);
				emit_jump(fout, label);
			}
		}
		else if (sym == 'A')
		...
		else if (sym == SYM_LOG_AND)
		{
			...
			{
				char label[MAX_LABEL_LENGTH];
				sprintf(label, "_%s_and_end%d", function_name, id);
				fprintf(fout, "\tCMP_X0_0              # &&\n");
				emit_branch(fout, "EQ", label);
				fprintf(fout, "\tPOP_X0\n");
			}
		...
		else if (sym == SYM_LOG_OR)
		{
			...
			{
				char label[MAX_LABEL_LENGTH];
				sprintf(label, "_%s_or_end%d", function_name, id);
				fprintf(fout, "\tCMP_X0_0              # ||\n");
				emit_branch(fout, "NE", label);
				fprintf(fout, "\tPOP_X0\n");
			}
			if (nesting_depth >= MAX_NESTING)
			{
